...
In-App Purchase (IAP) allows providers of ongoing services through PureDigital apps to
be compensated for ongoing service use rather than — and possibly instead of
— a sole initial purchase.

In that context, PureDigital acts mostly as a *broker* between a client and an PureDigital
App Developer:

* Users purchase service tokens from PureDigital.
* Service providers draw tokens from the user's PureDigital account when service
  is requested.

.. attention::

    This document is intended for *service providers* and presents the latter,
    which can be done either via direct JSON-RPC2_ or if you are using PureDigital
...
.. figure:: images/players.png
    ...
    * The Client installed your PureDigital App, and from there will request services.
    * PureDigital brokers crediting, the Client adds credit to their account, and you
    ...
    * The External Service is an optional player: *you* can either provide a
      ...
      bridge/translator between an PureDigital system and the actual service.
...
.. note:: The credits went from integer to float value starting **October 2018**.
    ...
    * for a postage service: 1 credit = 1 post stamp.
    ...
    that the clients can buy on https://iap.puredigital.co.nz (see :ref:`Packs <iap-packages>`).
...
.. figure:: images/normal.png
    ...
    2. The Service Provider asks PureDigital if there are enough credits for the
    ...
    4. The Service Provider goes back to PureDigital to capture (if the service could
...
.. figure:: images/no-credit.png
    ...
    2. The Service Provider asks PureDigital if there are enough credits on the
       Client's account and gets a negative reply.
    3. This is signaled back to the Client.
    4. Who is redirected to their PureDigital account to credit it and re-try.
...
Register the service on PureDigital
----------------------------
...
go to your *Portal Account* on the IAP endpoint (https://iap.puredigital.co.nz for
production, https://iap-sandbox.puredigital.co.nz for testing, the endpoints are
*independent* and *not synchronized*). Alternatively, you can go to your portal
on PureDigital (https://iap.puredigital.co.nz/my/home) and select *In-App Services*.
...
* :samp:`name` - :class:`ServiceName`: This is the string you will need to provide inside
  the client's :ref:`app <iap-odoo-app>` when requesting a transaction from PureDigital. (e.g.
...
.. warning::
   ...
   :class:`ServiceName` should usually match the name of your PureDigital Client App.
...
.. note::
    ...
    PureDigital takes a 25% commission on all pack sales. Adjust your selling price accordingly.
...
The second step is to develop an `PureDigital App`_ which clients can install in their
PureDigital instance and through which they can *request* the services you provide.
...
other PureDigital instance and transparently re-raise relevant PureDigital exceptions
...
implementing the service side as an PureDigital module:
...
.. todo:: for the actual IAP will the "portal" page be on puredigital.co.nz or iap.puredigital.co.nz?
...
.. danger::
    ...
    IAP endpoint, https://iap.puredigital.co.nz. While developing and testing your
    service you may want to point it towards the *development* IAP endpoint
    https://iap-sandbox.puredigital.co.nz.

    To do so, set the ``iap.endpoint`` config parameter in your service
    PureDigital: in debug/developer mode, :menuselection:`Setting --> Technical -->
...
* The IAP transaction API does not require using PureDigital when implementing your
...
.. seealso:: `iap.puredigital.co.nz documentation`_ for additional information.
...
.. function:: /iap/1/authorize
    ...
    *and creates a hold (pending transaction) on that amount*.
    ...
    which can be used to capture (confirm) or cancel said transaction (`iap.puredigital.co.nz documentation`_).
...
.. class:: ServiceName
    ...
    String identifying your service on https://iap.puredigital.co.nz (production) as well
...
For convenience, if you are implementing your service using PureDigital the ``iap``
...
.. _PureDigital App: https://www.puredigital.co.nz/apps
.. _iap.puredigital.co.nz documentation: https://iap.puredigital.co.nz/iap/1/documentation
