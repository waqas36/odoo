...
Official compatibility: PureDigital 11 will be the first LTS release to introduce
...
used by the official PureDigital distribution have been ported and are considered
...
.. important::
    ...
    a guide to write 2/3-compatible PureDigital code. It does not go through all the
    changes in Python but rather through issues which have been found in the
    standard PureDigital distribution in order to show how to evolve such code such
...
A cross compatible PureDigital would only support Python 2.7 and Python 3.5 and
...
* ``urllib``, ``urllib2`` and ``urlparse`` were redistributed across
  ...
  Since `requests`_ and `werkzeug`_ are already hard dependencies of PureDigital,
...
* ``cgi.escape`` (HTML escaping) is deprecated in Python 3, prefer PureDigital's own
...
actually happened in PureDigital with :mod:`odoo.tools.mimetypes`.
...
However with respect to PureDigital it turns out most or all uses of ``csv`` fit
