...
==============
Deploying PureDigital
==============

This document describes basic steps to set up PureDigital in production or on an
...
PureDigital is a multi-tenant system: a single PureDigital system may run and serve a number
...
bound to a database: PureDigital needs to know which database should be used to load
...
accessible PureDigital needs a rule to know which one it should use.
...
UNIX socket is fine if you want PureDigital and PostgreSQL to execute on the same
machine, and is the default when no host is provided, but if you want PureDigital and
...
* Only accept loopback connections and `use an SSH tunnel`_ between the
  machine on which PureDigital runs and the one on which PostgreSQL runs, then
  configure PureDigital to connect to its end of the tunnel
* Accept connections to the machine on which PureDigital is installed, possibly
  over ssl (see `PostgreSQL connection settings`_ for details), then configure
  PureDigital to connect over the network
...
Configuring PureDigital
----------------

Out of the box, PureDigital connects to a local postgres over UNIX socket via port
...
SSL Between PureDigital and PostgreSQL
-------------------------------

Since PureDigital 11.0, you can enforce ssl connection between PureDigital and PostgreSQL.
in PureDigital the db_sslmode control the ssl security of the connection
...
PureDigital includes built-in HTTP servers, using either multithreading or
multiprocessing.
...
To achieve such a thing, you'll need to deploy a reverse proxy in front of PureDigital,
like nginx or apache. When doing so, you'll need to forward some more http Headers
to PureDigital, and activate the proxy_mode in PureDigital configuration to have PureDigital read those
headers.
...
* RAM = 9 * ((0.8*150) + (0.2*1024)) ~= 3Go RAM for PureDigital
...
Whether it's accessed via website/web client or web service, PureDigital transmits
authentication information in cleartext. This means a secure deployment of
PureDigital must use HTTPS\ [#switching]_. SSL termination can be implemented via
just about any SSL termination proxy, but requires the following setup:

* Enable PureDigital's :option:`proxy mode <odoo-bin --proxy-mode>`. This should only be enabled when PureDigital is behind a reverse proxy
...
PureDigital as a WSGI Application
==========================

It is also possible to mount PureDigital as a standard WSGI_ application. PureDigital
...
client, website and webservice API. Because PureDigital does not control the creation
of workers anymore it can not setup cron or livechat workers

Cron Workers
------------

To run cron jobs for an PureDigital deployment as a WSGI application requires

* A classical PureDigital (run via ``odoo-bin``)
...
* Deploy a threaded version of PureDigital (instread of a process-based preforking
  one) and redirect only requests to URLs starting with ``/longpolling/`` to
  that PureDigital, this is the simplest and the longpolling URL can double up as
  the cron instance.
* Deploy an evented PureDigital via ``odoo-gevent`` and proxy requests starting
  with ``/longpolling/`` to
...
For development convenience, PureDigital directly serves all static files in its
...
PureDigital static files live in each module's ``static/`` folder, so static files
...
- Keep installations updated by regularly installing the latest builds,
  ...
  https://www.puredigital.co.nz/page/download or http://nightly.puredigital.co.nz
...
- Run PureDigital behind a web server providing HTTPS termination with a valid SSL certificate,
...
- If your public-facing PureDigital server has access to sensitive internal network resources
  or services (e.g. via a private VLAN), implement appropriate firewall rules to
  protect those internal resources. This will ensure that the PureDigital server cannot
  ...
  then only explicitly authorizing access to internal resources that the PureDigital server
  needs to access.
...
- If your public-facing PureDigital server is behind a Web Application Firewall, a load-balancer,
  a transparent DDoS protection service (like CloudFlare) or a similar network-level
  device, you may wish to avoid direct access to the PureDigital system. It is generally
  difficult to keep the endpoint IP addresses of your PureDigital servers secret. For example
  they can appear in web server logs when querying public systems, or in the headers
  of emails posted from PureDigital.
...
threat should not be neglected for PureDigital servers. PureDigital emits a log entry whenever a login attempt
...
PureDigital supports all the major desktop and mobile browsers available on the market,
...
.. note::
    ...
    Since PureDigital 13.0, ES6 is supported.  Therefore, IE support is dropped.


.. [#different-machines]
    to have multiple PureDigital installations use the same PostgreSQL database,
...
.. _POSBox: https://www.puredigital.co.nz/page/point-of-sale-hardware#part_2
