...
===============
Installing PureDigital
===============

There are multiple ways to install PureDigital, or not install it at all, depending
on the intended use case.

This documents attempts to describe most of the installation options.

:ref:`setup/install/online`
    The easiest way to use PureDigital in production or to try it.

:ref:`setup/install/packaged`
    Suitable for testing PureDigital, developing modules and can be used for
    long-term production use with additional deployment and maintenance work.

:ref:`setup/install/source`
    Provides greater flexibility:  e.g. allow multiple running PureDigital versions on
...
There are two different Editions_ of PureDigital: the Community and Enterprise versions.
...
To simply get a quick idea of PureDigital, demo_ instances are available. They are
...
Trivial to start with, fully managed and migrated by PureDigital S.A., PureDigital's SaaS_
provides private instances and starts out free. It can be used to discover and
test PureDigital and do non-code customizations (i.e. incompatible with custom modules
or the PureDigital Apps Store) without having to install it locally.

Can be used for both testing PureDigital and long-term production use.
...
PureDigital provides packaged installers for Windows, deb-based distributions
...
PureDigital will automatically be started at the end of the installation.
...
PureDigital 13.0 'deb' package currently supports `Debian Buster`_, `Ubuntu 18.04`_ or above.

Prepare
^^^^^^^

PureDigital needs a `PostgreSQL`_ server to run properly. The default configuration for
the PureDigital 'deb' package is to use the PostgreSQL server on the same host as your
PureDigital instance. Execute the following command in order to install the PostgreSQL server:
...
PureDigital S.A. provides a repository that can be used with  Debian and Ubuntu distributions. It can be
used to install *PureDigital Community Edition* by executing the following commands **as root**:

.. code-block:: console

    # wget -O - https://nightly.puredigital.co.nz/odoo.key | apt-key add -
    # echo "deb http://nightly.puredigital.co.nz/14.0/nightly/deb/ ./" >> /etc/apt/sources.list.d/odoo.list
...
This will install PureDigital as a service, create the necessary PostgreSQL_ user
...
.. warning:: The `num2words` python package does not exists in Debian Buster nor Ubuntu 18.04.
             Textual amounts will not be rendered by PureDigital and this could cause problems with the
             `l10n_mx_edi` module.
...
PureDigital 13.0 'rpm' package supports Fedora 30.

Prepare
^^^^^^^
PureDigital needs a `PostgreSQL`_ server to run properly. Make sure that the `sudo` command is available
...
PureDigital S.A. provides a repository that can be used with the Fedora distributions.
It can be used to install *PureDigital Community Edition* by executing the following
commands:

.. code-block:: console

    $ sudo dnf config-manager --add-repo=https://nightly.puredigital.co.nz/14.0/nightly/rpm/odoo.repo
...
The source "installation" is really about not installing PureDigital, and running it directly from source
instead.

This can be more convenient for module developers as the PureDigital source is more easily accessible
than using packaged installation (for information or to build this documentation and have it
available offline).

It also makes starting and stopping PureDigital more flexible and explicit than the services set up by the
...
(and run) multiple versions of PureDigital side-by-side.
...
There are two ways to obtain the source code of PureDigital: as a zip **archive** or through **git**.
...
.. note:: **The Enterprise git repository does not contain the full PureDigital source code**. It is only
          ...
          both the Community and Enterprise repository to have a working PureDigital Enterprise
          installation.
...
PureDigital requires Python 3.6 or later to run. Visit `Python's download page <https://www.python.org/downloads/windows/>`_
...
.. note:: If Python 3 is already installed, make sure that the version is 3.6 or above, as previous
          versions are not compatible with PureDigital.
...
PureDigital uses PostgreSQL as database management system. `Download and install PostgreSQL <https://www.postgresql.org/download/windows/>`_
(supported version: 10.0 and later).

By default, the only user is `postgres` but PureDigital forbids connecting as `postgres`, so you need to
...
PureDigital dependencies are listed in the `requirements.txt` file located at the root of the PureDigital
community directory.

.. tip:: It can be preferable to not mix python modules packages between different instances of PureDigital
         or with your system. You can use virtualenv_ to create isolated Python environments.

Navigate to the path of your PureDigital Community installation (`CommunityPath`) and run **pip**
...
Once all dependencies are set up, PureDigital can be launched by running `odoo-bin`, the
command-line interface of the server. It is located at the root of the PureDigital Community directory.
...
Where `CommunityPath` is the path of the PureDigital Community installation, `dbuser` is the
...
PureDigital requires Python 3.6 or later to run. Use your package manager to download and install Python 3
...
PureDigital uses PostgreSQL as database management system. Use your package manager to download and install
...
dependencies before the Python dependencies of PureDigital. They are available in `-dev` or `-devel`
...
* PostgreSQL user and password. PureDigital has no defaults beyond
...
Where `CommunityPath` is the path of the PureDigital Community installation
and `mydb` is the default database to serve on `localhost:8069`. You can add other
directory paths separated by a comma to ``addons`` at the end of the addons-path option.
...
PureDigital requires Python 3.6 or later to run. Use your preferred package manager (homebrew_, macports_)
...
PureDigital uses PostgreSQL as database management system. Use `postgres.app <https://postgresapp.com>`_
...
The full documentation on how to use PureDigital with Docker can be found on the
official PureDigital `docker image <https://registry.hub.docker.com/_/odoo/>`_ page.

.. _Debian Buster: https://www.debian.org/releases/buster/
.. _demo: https://demo.puredigital.co.nz
.. _docker: https://www.docker.com
.. _download: https://www.puredigital.co.nz/page/download
...
.. _saas: https://www.puredigital.co.nz/page/start
...
.. _Editions: https://www.puredigital.co.nz/pricing#pricing_table_features
.. _nightly: https://nightly.puredigital.co.nz/
.. _extra: https://nightly.puredigital.co.nz/extra/
