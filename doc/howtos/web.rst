...
This guide is about creating modules for PureDigital's web client.

To create websites with PureDigital, see :doc:`website`; to add business capabilities
or extend existing business systems of PureDigital, see :doc:`backend`.
...
.. warning::
    ...
    * `Underscore.js`_
    ...
    It also requires :ref:`an installed PureDigital <setup/install>`, and Git_.


A Simple Module
===============

Let's start with a simple PureDigital module holding basic web component
...
You then need to add that folder to PureDigital's
...
Files used in the "web" side of an PureDigital module must be placed in a ``static``
...
.. warning::
    ...
    "developer mode": log into your PureDigital instance (user *admin* password
    *admin* by default) open the user menu (in the top-right corner of the
    PureDigital screen) and select :guilabel:`About PureDigital` then :guilabel:`Activate
    the developer mode`:
...
The PureDigital framework uses one such pattern to define modules within web addons,
...
In PureDigital web, modules are declared as functions set on the global ``odoo``
...
* the first parameter is the current instance of the PureDigital web client, it gives
  access to various capabilities defined by the PureDigital (translations,
...
* the second parameter is your own local namespace automatically created by
  ...
  outside your module (either because the PureDigital web client needs to call them
...
For simplicity and developer-friendliness PureDigital web provides a class
...
The PureDigital web client bundles jQuery_ for easy DOM manipulation. It is useful
...
PureDigital Web makes specific components responsible for sections of a page. In
PureDigital web, the base for such components is the :class:`~odoo.Widget`
...
Let's add some content to the widget's root element, using jQuery::
    ...
    local.HomePage = instance.Widget.extend({
        start: function() {
            this.$el.append("<div>Hello dear PureDigital user!</div>");
        },
    });
...
.. note::
    ...
    to refresh the javascript code loaded in PureDigital Web, you will need to reload
    the page. There is no need to restart the PureDigital server.

The ``HomePage`` widget is used by PureDigital Web and managed automatically.
...
.. code-block:: html
    ...
    <div class="oe_petstore_homepage">
        <div>Hello dear PureDigital user!</div>
...
As many other environments, PureDigital's solution is to use a `template engine`_.
PureDigital's template engine is called :ref:`reference/qweb`.
...
* It has special support in PureDigital Web's :class:`~odoo.Widget`, though it
  can be used outside of PureDigital's web client (and it's possible to use
...
.. note::
    ...
    like PureDigital :ref:`views <reference/views>`.
    ...
    characteristic of PureDigital, and losing it was considered unacceptable.
...
.. note::
    ...
    the PureDigital web application.
...
The class system of the PureDigital web framework allows direct modification of
...
In PureDigital, translations files are automatically generated by scanning the source
...
Communication with the PureDigital Server
==================================

Contacting Models
-----------------

Most operations with PureDigital involve communicating with *models* implementing
...
communicating with PureDigital requires additional metadata whose setup before every
call would be verbose and error-prone. As a result, PureDigital web provides
...
The class used to call PureDigital models is :class:`odoo.Model`. It is
instantiated with the PureDigital model's name as first parameter
(``oepetstore.message_of_the_day`` here).

:func:`~odoo.web.Model.call` can be used to call any (public) method of an
PureDigital model. It takes the following positional arguments:
...
the time zone of the user, used to compute correctly dates and times if PureDigital
...
are related to the configuration of the current user in PureDigital plus the
...
While :func:`~odoo.Model.call` is sufficient for any interaction with PureDigital
models, PureDigital Web provides a helper for simpler and clearer querying of models
...
.. exercise:: Pet Toys List
    ...
    In PureDigital, images are generally stored in regular fields encoded as
...
In PureDigital, many operations start from an :ref:`action <reference/actions>`:
...
In PureDigital Web, the component responsible for handling and reacting to these
...
the PureDigital web know to start this widget? Because the widget is registered as
a *client action*.

A client action is (as its name implies) an action type defined almost
entirely in the client, in javascript for PureDigital web. The server simply sends
...
from the PureDigital web client root.
...
Much of PureDigital web's usefulness (and complexity) resides in views. Each view
...
Most :ref:`PureDigital views <reference/views>` are implemented through a subclass
...
The *search view* is considered a view type by the main PureDigital framework, but
...
* It must correctly implement the 3 field attributes available in all fields
  of PureDigital. The ``AbstractField`` class already implements an algorithm that
  ...
  * ``readonly``: When ``true``, the field must not be editable by the
    user. Most fields in PureDigital have a completely different behavior depending
...
* Fields have two methods, ``set_value()`` and ``get_value()``, which are
  ...
  given by the PureDigital server when a ``read()`` is performed on a model and give
  ...
  ``write()`` is not necessarily the same in PureDigital. As example, when you read a
...
interface and the ``AbstractField`` class directly in the code of the PureDigital web
client.
...
user to modify it can be useful, but most fields in PureDigital also allow editing.
