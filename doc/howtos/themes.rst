...
PureDigital celebrates freedom. Freedom for the designer to go further and
freedom for the user to customize everything according to their needs.

Ready to create your own theme? Great. Here are some things you should know before you begin. This tutorial is a guide to creating an PureDigital theme.
...
If you are a web designer using PureDigital for the first time, you are in the right place.
This introduction will outline the basics of PureDigital theme creation.

.. note::

   PureDigital’s team has created a framework that’s powerful and easy to use. There’s no need to know special syntaxes to use this set of tools.

From common CMS to PureDigital
-----------------------
...
This is usually the first question from a web designer used to working with Wordpress or Joomla and coming to PureDigital for the first time.
...
.. container:: row
  ...
  .. container:: col-sm-7
    ...
    This **does not** apply to creating PureDigital themes.
...
PureDigital default theme structure
----------------------------
...
.. container:: row
  ...
  .. container:: col-sm-8
    ...
    PureDigital comes with a default theme structure.
    ...
    It will upgrade automatically within your PureDigital installation and, since it is included in the Website Builder module, everything is smoothly integrated by default.
...
An PureDigital theme is not a folder containing HTML or PHP files, it’s a modular framework written in XML. Never worked with XML files before? Don’t worry, after following the tutorial, you’ll be able to create your first theme with only basic knowledge of HTML.
...
Creating themes for PureDigital is a total change of perspective. Instead of defining the complete layout for a page, you can create blocks (snippets) at let the user choose where to “drag&drop” them, creating the page layout on their own.
We call this modular design.

Imagine an PureDigital theme as a “list” of elements and options that you have to create and style.
...
.. container:: row
  ...
  .. figure:: theme_tutorial_assets/img/functionalities.jpg
     ...
     Thanks to PureDigital’s modularity, everything can be personalized even more. This means there are endless possibilities for your creativity. Adding functionalities is easy and it’s simple to provide the end user with customizable options.


PureDigital's XML files, an overview
-----------------------------

Any PureDigital XML file starts with encoding specifications.
...
the **PureDigital default structure**.  In order to do that you can use
...
PureDigital’s themes are packaged like modules. Even if you are designing a very simple website for your company or client, you need to package the theme like an PureDigital module.
...
These values will be used to identify your new theme in PureDigital’s backend.

The ``data`` property will contain the xml files list. Right now it’s empty, but we will add any new files created.

``category`` defines your module category (always “Theme”) and, after a slash, the subcategory. You can use one subcategory from the PureDigital Apps categories list. (https://www.puredigital.co.nz/apps/themes)
...
To install your theme, you just place your theme folder inside addons in your PureDigital installation.

After that, navigate to the PureDigital **Website** module, go to
:menuselection:`Configuration --> Settings`.

Under **Website** section click the **Pick a Theme** button, then hover over
your theme and click **Use this theme**.

Structure of an PureDigital page
=========================

An PureDigital page is the visual result of a combination of 2 kind of elements, **cross-pages** and **unique**.
By default, PureDigital provides you with a **Header** and a **Footer** (cross-pages) and a unique main element that contains the content that makes your page unique.
...
By default, PureDigital header contains a responsive navigation menu and the company’s logo. You can easily add new elements or style the existing one.

To do so, create a **layout.xml** file in your **views** folder and add the default PureDigital xml markup.
...
.. warning::
  ...
  your changes will take priority in any future PureDigital’s update.
...
default PureDigital markup.  Inside ``<odoo>``, instead of defining a ``<template>``,
...
Using ``<t t-call="website.layout">`` we will extend the PureDigital
...
PureDigital includes Bootstrap by default. This means that you can take advantage of all Bootstrap styles and layout functionalities out of the box.
...
Let’s navigate to the view folder and create an XML file called *assets.xml*. Add the default PureDigital xml markup and copy/paste the following code. Remember to replace ``theme folder`` with your theme’s main folder name.
...
attribute tells PureDigital that our template is referring to another one in
order to operate.
...
Our scss file is now included in our theme, it will be automatically compiled, minified and combined with all PureDigital’s assets.
...
Add the default PureDigital xml markup and copy/paste the following code.
...
We start by adding a new file in our views folder - name it **options.xml** and add the default PureDigital XML markup. Create a new template copy/pasting the following
