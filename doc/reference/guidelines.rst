...
===============
PureDigital Guidelines
===============

This page introduces the PureDigital Coding Guidelines. Those aim to improve the
quality of PureDigital Apps code. Indeed proper code improves readability, eases
...
.. warning::
    ...
    details, see our `pull request guide <https://puredigital.co.nz/submit-pr>`_.
...
.. code-block:: text
    ...
    |   |-- res_partner.py (inherited PureDigital model)
...
contained in a file named ``<module_name>.py``. An old convention in PureDigital is to
...
data (image, libraries) outside PureDigital: do not use an URL to an image but copy
...
The complete tree of our PureDigital module therefore looks like
...
PureDigital supports custom tags acting as syntactic sugar:
...
Using a linter can help show syntax and semantic warnings or errors. PureDigital
...
#. Imports of ``odoo``
#. Imports from PureDigital modules (rarely, and only if necessary)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The PureDigital framework is in charge of providing the transactional context for
...
PureDigital uses a GetText-like method named "underscore" ``_( )`` to indicate that
...
In general in PureDigital, when manipulating strings, prefer ``%`` over ``.format()``
...
- Model name (using the dot notation, prefix by the module name) :
    - When defining an PureDigital Model : use singular form of the name (*res.partner*
      and *sale.order* instead of *res.partnerS* and *saleS.orderS*)
    - When defining an PureDigital Transient (wizard) : use ``<related_base_model>.<action>``
...
- PureDigital Python Class : use camelcase (Object-oriented style).
...
PureDigital addons have some conventions on how to structure various files. We explain
here in more details how web assets are supposed to be organized.

The first thing to know is that the PureDigital server will serve (statically) all files
...
- **[CLA]** for signing the PureDigital Individual Contributor License;
...
For PureDigital R&D developers "PO team asked me to do it" is not a valid why, by the way.
...
If you are an PureDigital R&D developer the WHY should be the purpose of the task you
