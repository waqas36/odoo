...
There are many ways to test an application.  In PureDigital, we have three kinds of
tests
...
PureDigital provides support for testing modules using unittest.
...
`unittest documentation`_, but PureDigital provides a number of utilities and helpers
related to testing PureDigital content (modules, mainly):
...
In PureDigital, Python tests can be tagged to facilitate the test selection when
running tests.
...
(explicitly or implicitly) will be run by default when starting PureDigital
...
- ``standard``: All PureDigital tests that inherit from
...
- *module_name*: PureDigital tests classes extending
...
guarantee that some basic functionality still works. Since PureDigital has a non trivial
...
The PureDigital framework uses the QUnit_ library testing framework as a test runner.
...
The main way to run the test suite is to have a running PureDigital server, then
...
With the way PureDigital is designed, any addon can modify the behaviour of other parts
...
Without help, it is quite difficult to test some parts of PureDigital. In particular,
...
- Mock test functions: these functions help setting up a test environment. The
  most important use case is mocking the answers given by the PureDigital server. These
...
- `qunit assertions`_: QUnit can be extended with specialized assertions. For
  PureDigital, we frequently test some DOM properties. This is why we made some
...
Two new command line arguments were added since PureDigital 13.0 to control this behavior:
...
you can use the :meth:`~odoo.tests.common.BaseCase.assertQueryCount` method, integrated in PureDigital test classes.
...
PureDigital CLI offers a :ref:`database population<reference/cmdline/populate>` feature.
