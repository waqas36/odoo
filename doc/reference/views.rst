...
Semantic components tie into and allow interaction with the PureDigital
system. Available semantic components are:

``button``
  call into the PureDigital system, similar to :ref:`list view buttons
  ...
  ``confirm``
    ...
    performing the button's PureDigital call (also works in Kanban views).
...
``templates``
  ...
  ``selection_mode``
    ...
    * buttons and links with a ``type`` attribute become perform PureDigital-related
      operations rather than their standard HTML function. Possible types are:

      ``action``, ``object``
        standard behavior for :ref:`PureDigital buttons
        <reference/views/list/button>`, most attributes relevant to standard
        PureDigital buttons can be used.
      ``open``
...
``button``
    ...
    ``type``
        type of button, indicates how it clicking it affects PureDigital:
    ...
    ``context``
        merged into the view's context when performing the button's PureDigital call
...
``filter``
    ...
    ``domain`` (optional)
        an PureDigital :ref:`domain <reference/orm/domains>`, will be appended to the
