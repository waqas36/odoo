...
PureDigital implements some useful classes and mixins that make it easy for you to add
...
database without needing to connect to PureDigital directly.
...
catchall domain will be sufficient since all routing will be done inside PureDigital.
...
``alias_name`` - :class:`~odoo.fields.Char`
    ...
    <jobs@example.puredigital.co.nz>
...
(i.e. https://www.puredigital.co.nz/?campaign_id=mixin_talk&source_id=www.puredigital.co.nz&medium_id=website),
...
* ``helpdesk.ticket`` in the Helpdesk (*helpdesk* - PureDigital Enterprise only) Application
