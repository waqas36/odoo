...
aims to provide PureDigital developers with the knowledge required
...
.. note:: Timezones
    ...
    in the UTC timezone. This is by design, as it makes the PureDigital database independent from the timezone
...
.. attribute:: company_id
  ...
  Main field name used for PureDigital multi-company behavior.
...
PureDigital maintains a cache for the fields of the records, so that not every field
...
To avoid reading one field on one record at a time, PureDigital *prefetches* records
...
.. warning::
    Executing raw SQL bypasses the ORM, and by consequent, PureDigital security rules.
...
PureDigital provides three different mechanisms to extend models in a modular way:
...
:attr:`~odoo.models.Model._name` attributes together, PureDigital creates a new
