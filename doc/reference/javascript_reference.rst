...
This document presents the PureDigital Javascript framework. This
...
- the *website*: this is the public part of PureDigital.  It allows an unidentified
...
It means that while a user is working on PureDigital, the web client class (and the
...
Managing assets in PureDigital is not as straightforward as it is in some other apps.
...
a collection of files (javascript, css, scss). In PureDigital, the most important
...
Main bundles
------------
When the PureDigital server is started, it checks the timestamp of each file in a bundle,
...
sure they are loaded in the correct order.  In order to do that, PureDigital has defined
a small module system (located in the file *addons/web/static/src/js/boot.js*,
which needs to be loaded first).

The PureDigital module system, inspired by AMD, works by defining the function *define*
on the global odoo object. We then define each javascript module by calling that
function.  In the PureDigital framework, a module is a piece of code that will be executed
...
PureDigital was developed before ECMAScript 6 classes were available.  In Ecmascript 5,
...
For these reasons, PureDigital decided to use its own class system, inspired by John
...
the way PureDigital is structured, it is sometimes necessary in one addon to modify
...
* Global selectors should be avoided. Because a component may be used several
  times in a single page (an example in PureDigital is dashboards), queries should be
  ...
  lead to unintended or incorrect behavior.  PureDigital Web's
...
There are currently two event systems supported by PureDigital: a simple system which
...
- *trigger_up*: this is the method that will create a small *PureDigitalEvent* and
...
The PureDigitalEvent class is very simple.  It has three public attributes: *target*
...
A common need in the PureDigital ecosystem is to extend/change the behaviour of the
...
There are typically two usecases when working on PureDigital: one may need to call a
...
The PureDigital framework has a standard way to communicate various information to the
...
The notification system in PureDigital is designed with the following components:
...
In 2017, PureDigital replaced the previous view code with a new architecture.  The
