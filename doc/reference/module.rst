...
The manifest file serves to declare a python package as an PureDigital module
...
``license`` (``str``, defaults: ``LGPL-3``)
    ...
    * `OEEL-1` (PureDigital Enterprise Edition License v1.0)
    * `OPL-1` (PureDigital Proprietary License v1.0)
    * `Other proprietary`

``category`` (``str``, default: ``Uncategorized``)
    classification category within PureDigital, rough business domain for the module.
...
``depends`` (``list(str)``)
    PureDigital modules which must be loaded before this one, either because this
