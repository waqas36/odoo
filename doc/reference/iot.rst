...
IoT Drivers allow any PureDigital module to communicate in real-time with any device
connected to the IoT Box. Communication with the IoT Box goes both ways, so the
PureDigital client can send commands to and receive information from any of the
supported devices.
...
be located on the connected PureDigital instance. Each module can contain an
...
through its IP address and in the IoT module of the connected PureDigital instance.
...
from an PureDigital module. It takes the data given during the call as argument.
...
When any module in PureDigital wants to read data from a specific device, it creates a
