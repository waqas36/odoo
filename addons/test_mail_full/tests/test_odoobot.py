...
@tagged("odoobot")
class TestPureDigitalbot(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestPureDigitalbot, cls).setUpClass()
        ...
        cls.odoobot_ping_body = '<a href="http://puredigital.co.nz/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@PureDigitalBot</a>' % (cls.odoobot.id, cls.odoobot.id)
    ...
    def test_odoobot_ping(self):
        ...
        # PureDigitalbot should not be a follower but user_employee and user_admin should
