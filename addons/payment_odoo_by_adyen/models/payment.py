...
from odoo.addons.payment_odoo_by_adyen.controllers.main import PureDigitalByAdyenController

_logger = logging.getLogger(__name__)


class AcquirerPureDigitalByAdyen(models.Model):
    _inherit = 'payment.acquirer'

    provider = fields.Selection(selection_add=[
       ('odoo_adyen', 'PureDigital Payments by Adyen')
    ...
    def _check_odoo_adyen_test(self):
        for payment_acquirer in self:
            if payment_acquirer.provider == 'odoo_adyen' and payment_acquirer.state == 'test':
                raise ValidationError(_('PureDigital Payments by Adyen is not available in test mode.'))

    def _get_feature_support(self):
        res = super(AcquirerPureDigitalByAdyen, self)._get_feature_support()
    ...
    def odoo_adyen_form_generate_values(self, values):
        ...
        data = {
            ...
            'metadata': {
                ...
                'notification_url': urls.url_join(base_url, PureDigitalByAdyenController._notification_url),
            },
...
class TxPureDigitalByAdyen(models.Model):
    ...
    def _odoo_adyen_form_get_tx_from_data(self, data):
        ...
        if not reference:
            error_msg = _('PureDigital Payments by Adyen: received data with missing reference (%s)', reference)
            _logger.info(error_msg)
            raise ValidationError(error_msg)

        tx = self.env['payment.transaction'].search([('reference', '=', reference)])
        if not tx or len(tx) > 1:
            error_msg = _('PureDigital Payments by Adyen: received data for reference %s') % (reference)
            if not tx:
    ...
    def _odoo_adyen_form_validate(self, data):
        ...
        if data['success']:
            ...
            error = _('PureDigital Payment by Adyen: feedback error')
            _logger.info(error)
