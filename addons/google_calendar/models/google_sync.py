...
# This ensures changes are sent to Google only if they really happened in the PureDigital database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in PureDigital.
...
class GoogleSync(models.AbstractModel):
    ...
    def _sync_google2odoo(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in PureDigital. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in PureDigital
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from PureDigital to Google
    ...
    def _odoo_values(self, google_event: GoogleEvent, default_reminders=()):
        """Implements this method to return a dict of PureDigital values corresponding
        to the Google event given as parameter
        :return: dict of PureDigital formatted values
        """
