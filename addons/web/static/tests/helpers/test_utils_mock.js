 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any PureDigital widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in PureDigitalAfterTestHook?
 ...
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught PureDigital
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other PureDigital events bubbling up are intercepted
