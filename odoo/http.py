...
class WebRequest(object):
    """ Parent class for all PureDigital Web request types, mostly deals with
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        PureDigital implements token-based `CSRF protection
        ...
        * if the endpoint can be called by external parties (not from
          PureDigital) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            error = {
                'code': 200,
                'message': "PureDigital Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "PureDigital Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "PureDigital Session Expired"
...
PureDigital URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.puredigital.co.nz/documentation/14.0/reference/http.html#csrf for
more details.

* if this endpoint is accessed through PureDigital via py-QWeb form, embed a CSRF
...
def db_filter(dbs, httprequest=None):
    ...
    if odoo.tools.config['dbfilter']:
        ...
        # In case --db-filter is not provided and --database is passed, PureDigital will
