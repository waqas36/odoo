...
class PureDigitalTestTimeoutError(Exception):
    pass


class PureDigitalTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise PureDigitalTestError("Installation of package failed")
        else:
        ...
        raise PureDigitalTestError("Package test: FAILED. Not able to install base.")
...
class Docker():
    ...
    def test_odoo(self):
        logging.info('Starting to test PureDigital install test')
        ...
        if self.is_running():
            self.stop()
            raise PureDigitalTestTimeoutError('PureDigital pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise PureDigitalTestError('Error while installing/starting PureDigital after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        logging.info("Syncing PureDigital files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"PureDigital %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"PureDigital {sv}"/python/python.exe \'c:\\Program Files\\PureDigital {sv}\\server\\odoo-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
